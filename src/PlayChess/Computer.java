...
public class Computer { 
	...
	public static void newGame() {
		...
		compLocs=new ArrayList<int[]>();
		humanLocs=new ArrayList<int[]>();
		humans=0;
		comps=0;
	}
	...
	public Board makeMove(Board board) throws InterruptedException{
		...
		if(danger.isEmpty()){
			...
			//attack
			//Move move = dilemmaDanger(board);

			move= canAttackFree(board);
			...
			move=dangerAttackFree(board);
			if(move==null)
				move=dangerCanKillHigher(board);
			if(move==null)
				move=defend(board);
		}
		if(board.getPieceColor(move.mov)==Board.pieceColor.Black){
			captured.add(board.getPiece(move.mov));
			Display.updateCaptured(Play.comp);
		}
	...
	/*public Move dillemmaDanger(Board board){
		Move move;
		ArrayList<Move> allMoves= allMoves(board, humType);
