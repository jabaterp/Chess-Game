...
public class Human {
	...
	static ArrayList<Board.piece> captured = new ArrayList<Board.piece>();
	static int points;
	final static int empty=0,friend=1,enemy=2;
	static boolean leftRookOrKingMoved=false;
	static boolean rightRookOrKingMoved=false;
	
	public static void newGame() {
		captured=new ArrayList<Board.piece>();
		points=0;
		leftRookOrKingMoved=false;
		rightRookOrKingMoved=false;
	}

	public static Board move(Board board,int[] loc,int[] move) throws InterruptedException{
		int emptyOrEnemyVal=emptyOrEnemy(board,move);
		if (emptyOrEnemyVal==enemy||emptyOrEnemyVal==empty){
			if (canMovePiece(board,loc,move)){
				if(board.getPiece(loc)==Board.piece.bKing){
					leftRookOrKingMoved=true;
					rightRookOrKingMoved=true;
				}else if(board.getPiece(loc)==Board.piece.bRook && loc[1]==0){
					leftRookOrKingMoved=true;
				}else if(board.getPiece(loc)==Board.piece.bRook && loc[1]==7){
					rightRookOrKingMoved=true;
				}
				if(emptyOrEnemyVal==enemy){
					captured.add(board.getPiece(move));
					Display.updateCaptured(Play.human);
				}
	...
	public static boolean canMovePiece(Board board,int[] loc, int[] move){
		...
		case bKing:
			if (((((Math.abs(loc[0]-move[0])==1) && (Math.abs(loc[1]-move[1])==1))||
					...
					(!leftRookOrKingMoved && board.getPiece(loc)==Board.piece.bKing && move[0]==0 && move[1]==1) ||
					(!rightRookOrKingMoved && board.getPiece(loc)==Board.piece.bKing && move[0]==0 && move[1]==7) )
				return true;
			else return false;
		case bKnight: 
	...
	public static void castle(Board board){
